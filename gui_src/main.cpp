...
static void libusb_error(int errno1, const char *detailedText)
{
	char msg[30];
	char tbuf[60];
	...
	memset(tbuf,'\0',60);
	QMessageBox mb;
    if ( errno1 == LIBUSB_ERROR_IO )
		strcpy(msg, "LIBUSB_ERROR_IO");
    else if ( errno1 == LIBUSB_ERROR_INVALID_PARAM )
		strcpy(msg, "LIBUSB_ERROR_INVALID_PARAM" );
    else if ( errno1 == LIBUSB_ERROR_ACCESS )
		strcpy(msg, "LIBUSB_ERROR_ACCESS");
    else if ( errno1 == LIBUSB_ERROR_NO_DEVICE )
		strcpy(msg, "LIBUSB_ERROR_NO_DEVICE");
    else if ( errno1== LIBUSB_ERROR_NOT_FOUND )
		strcpy(msg, "LIBUSB_ERROR_NOT_FOUND");
    else if ( errno1 == LIBUSB_ERROR_BUSY )
		strcpy(msg, "LIBUSB_ERROR_BUSY");
    else if ( errno1 == LIBUSB_ERROR_TIMEOUT )
		strcpy(msg, "LIBUSB_ERROR_TIMEOUT");
    else if ( errno1 == LIBUSB_ERROR_OVERFLOW )
		strcpy(msg, "LIBUSB_ERROR_OVERFLOW");
    else if ( errno1 == LIBUSB_ERROR_PIPE )
		strcpy(msg, "LIBUSB_ERROR_PIPE");
    else if ( errno1 == LIBUSB_ERROR_INTERRUPTED )
		strcpy(msg, "LIBUSB_ERROR_INTERRUPTED");
    else if ( errno1 == LIBUSB_ERROR_NO_MEM )
		strcpy(msg, "LIBUSB_ERROR_NO_MEM");
    else if ( errno1 == LIBUSB_ERROR_NOT_SUPPORTED )
		strcpy(msg, "LIBUSB_ERROR_NOT_SUPPORTED");
    else if ( errno1 == LIBUSB_ERROR_OTHER )
		strcpy(msg, "LIBUSB_ERROR_OTHER");
	else strcpy(msg, "LIBUSB_ERROR_UNDOCUMENTED");

    sprintf(tbuf,"LIBUSB_ERROR NO : %d, %s",errno1,msg);
	mb.setText(tbuf);
...
void ControlCenter::on_streamer_control_start_clicked ()
{
    ...
    QString tempQ=mainwin->streamer_ep_sel->currentText();
    bool hasin=tempQ.contains("IN");
